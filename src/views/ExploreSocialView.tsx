...
.explore-v2-section{margin:0 0 30px}.explore-v2-head{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:14px}.explore-v2-head h2{margin:0;color:#151c43;font-size:28px;line-height:1.1}.explore-v2-head span,.explore-v2-head button{color:#7447f4;font-weight:850;border:0;background:transparent;white-space:nowrap}
.people-add-strip{display:grid;grid-auto-flow:column;grid-auto-columns:minmax(172px,1fr);gap:12px;overflow-x:auto;padding:2px 42px 7px 1px;scroll-snap-type:x mandatory;scroll-padding-right:42px;overscroll-behavior-inline:contain;scrollbar-width:none;-webkit-mask-image:linear-gradient(90deg,#000 0,#000 calc(100% - 28px),transparent 100%);mask-image:linear-gradient(90deg,#000 0,#000 calc(100% - 28px),transparent 100%)}.people-add-strip::-webkit-scrollbar{display:none}.people-add-card{scroll-snap-align:start;display:grid;justify-items:center;gap:8px;padding:16px 12px;border:1px solid rgba(116,71,244,.12);border-radius:24px;background:rgba(255,255,255,.95);box-shadow:0 13px 30px rgba(60,42,115,.08);min-width:0}.people-add-avatar{width:82px;height:82px;border-radius:50%;object-fit:cover;background:#eee9fb}.people-add-card strong{font-size:18px;color:#161d44}.people-add-card small{color:#81879a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:100%;display:flex;align-items:center;gap:4px}.people-add-card button{width:100%;height:36px;border-radius:999px;border:1px solid #7a4cf4;background:#fff;color:#7143ef;font-size:14px;font-weight:850;display:flex;align-items:center;justify-content:center;gap:6px;padding:0 10px}.people-add-card button svg{width:17px;height:17px;stroke-width:2}.people-add-card button.connected{background:#7547f4;color:#fff}.people-add-empty{padding:18px;border-radius:20px;background:#fff;color:#777e91}
.story-v2-strip{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;align-items:start;padding:2px 0 8px;overflow:visible}.story-v2-chip{appearance:none;border:0;background:transparent;padding:0;min-width:0;text-align:center;color:inherit}.story-v2-ring{width:72px;height:72px;margin:0 auto;border-radius:50%;padding:3px;background:linear-gradient(135deg,#7347f4,#bd6ae8);display:grid;place-items:center;box-sizing:border-box}.story-v2-ring>img,.story-v2-ring>video{display:block;width:100%;height:100%;border-radius:50%;object-fit:cover;border:3px solid #fff;background:#e9e2fb;box-sizing:border-box}.story-v2-add .story-v2-ring{padding:0;background:#fff;border:2px dashed #7b4cf4;color:#7647f4}.story-v2-add .story-v2-ring svg{width:30px;height:30px;stroke-width:2}.story-v2-chip strong{display:block;margin-top:8px;font-size:13px;line-height:1.1;color:#1e2448;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-shadow:none}.story-v2-chip small{display:block;color:#9298a8;font-size:11px;line-height:1.15;margin-top:3px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.story-v2-chip:focus-visible .story-v2-ring{outline:3px solid rgba(116,71,244,.28);outline-offset:3px}
...
@media(max-width:430px){.explore-social-v2{padding-bottom:108px}.explore-v2-hero{padding-top:4px}.explore-v2-head h2{font-size:25px}.people-add-strip{grid-auto-columns:158px;padding-right:38px;scroll-padding-right:38px}.people-add-card{padding:14px 10px}.people-add-card button{height:34px;font-size:13px}.story-v2-strip{gap:7px}.story-v2-ring{width:66px;height:66px}.story-v2-chip strong{font-size:12px}.story-v2-chip small{font-size:10px}.community-card{min-height:220px}}
@media(max-width:360px){.story-v2-strip{gap:4px}.story-v2-ring{width:60px;height:60px}.story-v2-chip strong{font-size:11px}.story-v2-chip small{font-size:9px}}
`;
  ...
  return <div className="page explore-page explore-social-v2"><style>{css}</style>
    ...
    <section className="explore-v2-section"><div className="explore-v2-head"><h2>Estados</h2><button type="button" onClick={()=>fileRef.current?.click()} disabled={uploading}>{uploading?'Subiendo…':'Subir estado'}</button></div>
      ...
      <div className="story-v2-strip">
        <button className="story-v2-chip story-v2-add" type="button" onClick={()=>fileRef.current?.click()} aria-label="Subir estado"><span className="story-v2-ring"><Plus/></span><strong>Subir estado</strong><small>Foto o vídeo</small></button>
        {storyMedia.map(item=><div className="story-v2-chip" key={item.id}><span className="story-v2-ring">{item.mediaType==='video'?<video src={item.url} muted playsInline/>:<img src={item.url} alt="Tu estado"/>}</span><strong>Tu estado</strong><small>Ahora</small></div>)}
        {storyPeople.map(person=><div className="story-v2-chip" key={person.name}><span className="story-v2-ring"><img src={person.image} alt={person.name}/></span><strong>{person.name}</strong><small>Reciente</small></div>)}
      </div>
    </section>
