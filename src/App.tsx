...
export default function App(){
  ...
  useEffect(()=>{let active=true;void Promise.allSettled([fetchRealPlans(),fetchSharedPlans()]).then(results=>{if(!active)return;const real=results[0].status==='fulfilled'?results[0].value:[];const shared=results[1].status==='fulfilled'?results[1].value:[];if(results[0].status==='rejected')console.warn('CONECTA real plans load failed; demo/shared plans kept',results[0].reason);if(results[1].status==='rejected')console.warn('CONECTA shared plans load failed; local demo remains available',results[1].reason);setCreatedPlans(local=>mergePlans(real,mergePlans(shared,local)));});return()=>{active=false};},[]);
