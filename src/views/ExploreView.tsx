...
import { Heart, Image as ImageIcon, MessageCircle, Plus, Search, Users, Zap } from 'lucide-react';
...
export function ExploreView({onPlan,extraPlans=[],initialFilter='all',initialCategory=null,onChat}:{onPlan:(plan:Plan)=>void;extraPlans?:Plan[];initialFilter?:ExploreFilter;initialCategory?:string|null;onChat:(contact:ChatContact)=>void;}){
 const [realPeople,setRealPeople]=useState<PersonSearchResult[]>([]); const [query,setQuery]=useState('');
 ...
 const plans=useMemo(()=>extraPlans.filter(plan=>planMatchesFilter(plan,initialFilter)).filter(plan=>!query.trim()||normalize(`${plan.title} ${plan.place} ${plan.category}`).includes(normalize(query))).slice(0,20),[extraPlans,initialFilter,query]);
 return <main className="page explore-page explore-live" aria-label="Explora">
  <header className="explore-visual-hero"><div><h1>Explora</h1><p>Conoce gente, descubre planes y vive nuevas experiencias</p></div></header><div className="explore-search-real"><Search/><input aria-label="Buscar planes" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar personas, planes, lugares…"/></div>
