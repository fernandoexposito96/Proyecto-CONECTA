...
const unsplash = (photoId: string) =>
  `https://images.unsplash.com/${photoId}?auto=format&fit=crop&w=1100&q=84`;
...
const realPhotos = {
  ...
  cycling: unsplash("photo-1541625602330-2277a4c46182"),
  cafe: unsplash("photo-1501339847302-ac426a4a7cbb"),
  brunch: unsplash("photo-1493770348161-369560ae357d"),
  ...
  games: unsplash("photo-1606167668584-78701c57f13d"),
} as const;
...
export const categories = [
  ...
  { label: "Ciclismo", icon: Bike, color: "#16a34a", image: realPhotos.cycling, images: photoPool(realPhotos.cycling, realPhotos.coast, realPhotos.travel) },
  { label: "Senderismo", icon: Mountain, color: "#84cc16", image: realPhotos.hiking, images: photoPool(realPhotos.hiking, realPhotos.coast, realPhotos.travel) },
  ...
  { label: "Café", icon: Coffee, color: "#3b82f6", image: realPhotos.cafe, images: photoPool(realPhotos.cafe, realPhotos.brunch, realPhotos.friends) },
  { label: "Gastronomía", icon: Soup, color: "#f97316", image: realPhotos.food, images: photoPool(realPhotos.food, realPhotos.brunch, realPhotos.city) },
  ...
  { label: "Juegos", icon: Gamepad2, color: "#06b6d4", image: realPhotos.games, images: photoPool(realPhotos.games, realPhotos.group, realPhotos.cafe) },
  { label: "Idiomas", icon: Languages, color: "#0ea5e9", image: realPhotos.group, images: photoPool(realPhotos.group, realPhotos.students, realPhotos.learning) },
  { label: "Viajes", icon: Plane, color: "#8b5cf6", image: realPhotos.travel, images: photoPool(realPhotos.travel, realPhotos.coast, realPhotos.city) },
  ...
  { label: "Familias", icon: UsersRound, color: "#ec4899", image: realPhotos.friends, images: photoPool(realPhotos.friends, realPhotos.group, realPhotos.coast) },
  ...
  { label: "Nuevos en la ciudad", icon: Sparkles, color: "#f43f5e", image: realPhotos.city, images: photoPool(realPhotos.city, realPhotos.group, realPhotos.cafe) },
...
export function categoryImage(category?: string | null, variant?: string | number | null) {
  const item = categories.find((candidate) => candidate.label === category) ?? categories[0];
  const images = item.images;
  const index = variant == null ? categoryCounters.get(item.label) ?? 0 : hashSeed(variant);
  if (variant == null) categoryCounters.set(item.label, index + 1);
  return images[index % images.length] ?? fallbackImage;
}

export function visualFallback(label = "CONECTA", variant: string | number = label) {
  const normalized = label.toLowerCase();
  const direct = categories.find((candidate) => normalized.includes(candidate.label.toLowerCase()));
  if (direct) return direct.images[hashSeed(variant) % direct.images.length] ?? fallbackImage;
  ...
  return generic[hashSeed(variant) % generic.length] ?? fallbackImage;
}

export function categoryColor(category?: string | null) {
  return categories.find((item) => item.label === category)?.color ?? "#8b5cf6";
}
