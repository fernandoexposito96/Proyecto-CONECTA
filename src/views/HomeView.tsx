...
import { CalendarDays, ChevronRight, Coffee, CreditCard, Heart, MapPin, Music2, Navigation, Sparkles, Users, Utensils, Dumbbell } from 'lucide-react';
...
export function HomeView({setView,onPlan,onBrowse}:{setView:(v:View)=>void,onPlan:(p:Plan)=>void,onBrowse:(mode:HomeBrowseMode,category?:string|null)=>void}){
  ...
  useEffect(()=>{
    let active=true;
    void loadSocialSummary().then(value=>{if(active)setSummary(value)}).catch(error=>console.warn('CONECTA next plan unavailable; demo remains available elsewhere',error)).finally(()=>{if(active)setSummaryReady(true)});
    return ()=>{active=false};
  },[]);
  ...
  const featuredPlan:Plan|null=summaryReady&&summary.nextPlan?{backendId:summary.nextPlan.id,title:summary.nextPlan.title,image:plans[5]?.image||plans[0].image,time:nextPlanDate(summary.nextPlan.startsAt),startsAt:summary.nextPlan.startsAt,place:summary.nextPlan.location||'Centre de Reus',distance:'',spots:'Plan confirmado',category:summary.nextPlan.category||'Plan'}:null;
  ...
  const compatiblePeople=[
    ...
    {name:'Javi',age:28,match:'88%',tags:['Deporte','Naturaleza'],image:allowedPeople.find(person=>person.name==='Javi')?.image||people[1].image},
  ...
  return <div className="page home-page home-target">
    ...
    <section className="home-map-card" aria-label="Planes cerca de ti">
      <div className="home-map-copy"><small>A TU ALREDEDOR</small><h2>Planes cerca de ti</h2><p>{locationAllowed?'Descubre qué está pasando ahora en Tarragona y alrededores':'Activa la ubicación para descubrir planes a tu alrededor'}</p></div>
      <button className="home-map-full" type="button" onClick={()=>browse(locationAllowed?'near':'all')}>Ver mapa completo <ChevronRight/></button>
      <div className="home-map-surface" aria-hidden="true"><span className="map-road road-a"/><span className="map-road road-b"/><span className="map-road road-c"/><b className="map-city map-reus">Reus</b><b className="map-city map-vila">Vila-seca</b><b className="map-city map-tgn">Tarragona</b><b className="map-city map-salou">Salou</b><i className="map-sea"/></div>
      <span className="home-map-pin pin-music"><Music2/></span><span className="home-map-pin pin-food"><Utensils/></span><span className="home-map-pin pin-sport"><Dumbbell/></span><span className="home-map-pin pin-social"><Users/></span><span className="home-map-user"/>
      <button className="home-map-distance" type="button" onClick={()=>browse(locationAllowed?'near':'all')}><Navigation/> {locationAllowed?'A 2 km de ti':'Ver planes cercanos'} <ChevronRight/></button>
      <button className="home-map-people" type="button" onClick={()=>browse(locationAllowed?'near':'all')}><span><img src={people[0].image} alt=""/><img src={people[2].image} alt=""/><img src={people[1].image} alt=""/><b>+12</b></span><em>Planes hoy cerca de ti</em><ChevronRight/></button>
    </section>

    <section className="premium-banner premium-card-version"><div><CreditCard/><span>CONECTA PREMIUM</span></div><div className="premium-mini-card"><CreditCard/><strong>CONECTA<br/>PREMIUM</strong></div><h2>Más planes, más conexiones</h2><p>Planes verificados · Recomendaciones avanzadas · Más seguridad</p><button onClick={()=>setView('Ajustes')}>Ver CONECTA Premium <ChevronRight/></button></section>
  </div>
}
