...
import { joinPlan, leavePlan, getPlanMembershipStatus } from '../lib/attendanceBackend';
...
export function PlanDetail({plan,onClose,onOpenChat,standalone=false}:{plan:Plan,onClose:()=>void,onOpenChat?:(name:string,planId?:string)=>void;standalone?:boolean}){
  ...
  const organizerName=social?.organizer?.name||(plan.backendId?plan.organizerName||'Organizador CONECTA':demoOrganizer);
  const organizerAvatar=social?.organizer?.avatar||plan.organizerAvatar||(plan.backendId?'./image-fallback.svg':'./assets/images/photo-1494790108377-be9c29b29330.jpg');
  ...
  useEffect(()=>{
    ...
    let active=true;
    void Promise.all([getPlanMembershipStatus(plan.backendId),loadPlanSocialDetails(plan.backendId)])
      .then(([joinedValue,socialValue])=>{if(active){setJoined(Boolean(joinedValue));setMembershipStatus(joinedValue);setSocial(socialValue)}})
  ...
  const toggleJoin=async()=>{
    ...
    const next=!joined;
    try{
      if(next)setMembershipStatus(await joinPlan(plan.backendId)); else {await leavePlan(plan.backendId);setMembershipStatus(null);}
      setJoined(next);
