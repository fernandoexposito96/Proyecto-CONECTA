...
export function SheetTrigger({ asChild: _asChild, children }: { asChild?: boolean; children: ReactElement }) { const context = useContext(SheetContext); if (!isValidElement(children)) return children; return cloneElement(children, { onClick: () => context?.setOpen(true) } as Record<string, unknown>); }
export function SheetContent({ className = "", side: _side, children }: { className?: string; side?: string; children: ReactNode }) { const context = useContext(SheetContext); if (!context?.open) return null; return <div className="sheet-overlay" role="presentation" onMouseDown={() => context.setOpen(false)}><aside className={`sheet-panel ${className}`} role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" type="button" aria-label="Cerrar" onClick={() => context.setOpen(false)}><X /></button>{children}</aside></div>; }
...
export function AlertDialogTrigger({ asChild: _asChild, children }: { asChild?: boolean; children: ReactElement }) { const context = useContext(AlertContext); return isValidElement(children) ? cloneElement(children, { onClick: () => context?.setOpen(true) } as Record<string, unknown>) : children; }
...
export function AlertDialogAction({ children, onClick, variant: _variant }: { children: ReactNode; onClick?: () => void; variant?: string }) { const context = useContext(AlertContext); return <button type="button" className="danger-action" onClick={() => { onClick?.(); context?.setOpen(false); }}>{children}</button>; }
...
export function InputOTP({ maxLength, value, onChange, children: _children }: { maxLength: number; value: string; onChange: (value: string) => void; children?: ReactNode }) { return <input className="otp-input" inputMode="numeric" autoComplete="one-time-code" maxLength={maxLength} value={value} onChange={(event) => onChange(event.target.value.replace(/\D/g, "").slice(0, maxLength))} aria-label="Código de verificación" />; }
export function InputOTPGroup({ children: _children }: { children: ReactNode }) { return null; }
export function InputOTPSlot({ index: _index }: { index: number }) { return null; }
