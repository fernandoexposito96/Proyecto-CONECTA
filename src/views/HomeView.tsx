...
export function HomeView({setView,onPlan,onBrowse}:{setView:(v:View)=>void,onPlan:(p:Plan)=>void,onBrowse:(mode:HomeBrowseMode,category?:string|null)=>void}){
  ...
  return <div className="page home-page home-target">
    ...
    <section className="home-target-section"><div className="home-target-head"><h2>Explora por categorías</h2><button onClick={()=>browse('categories')}>Ver todas <ChevronRight/></button></div><div className="home-target-categories">{homeCategories.map(([name,image])=><button key={name} onClick={()=>browse('categories',name)}><img src={image} alt={name}/><span/><b><CategoryIcon name={name}/>{name}</b></button>)}</div></section>
    ...
    <section className="home-target-section"><div className="home-target-head home-target-head-sub"><div><h2>Escapadas y eventos</h2><p>Planes más allá de tu ciudad</p></div><button onClick={()=>browse('escapes')}>Ver todas <ChevronRight/></button></div><div className="home-target-escapes">{homeEscapes.map(item=><article key={item.title} role="button" tabIndex={0} onClick={()=>openEscape(item)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();openEscape(item)}}}><img src={item.image} alt={item.title}/><span className="home-target-card-shade"/><Heart className="home-target-heart"/><div><strong>{item.title}</strong><span><MapPin/>{item.subtitle}</span></div></article>)}</div></section>
    ...
    <section className="home-target-section"><div className="home-target-head home-target-head-sub"><div><h2>Personas compatibles</h2><p>Gente con tus mismos intereses</p></div><button onClick={()=>browse('people')}>Ver todas <ChevronRight/></button></div><div className="home-target-people">{compatiblePeople.map(person=><article key={person.name}><div className="home-target-person-photo"><img src={person.image} alt={person.name}/><b>{person.match}</b></div><strong><i/>{person.name}, {person.age}</strong><span>{person.tags.join(' · ')}</span></article>)}</div></section>
