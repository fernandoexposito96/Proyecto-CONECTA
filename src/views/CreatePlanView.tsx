...
export function CreatePlanView({setView,onCreate}:{setView:(v:View)=>void,onCreate:(plan:Plan)=>Promise<boolean>}){
  ...
  const [description,setDescription]=useState(''); const [cost,setCost]=useState(''); const [created,setCreated]=useState(false); const [synced,setSynced]=useState(false); const [creating,setCreating]=useState(false);
  ...
  return <div className="page create-plan-page create-plan-final cp-simple">
    <header className="cp-head cp-simple-head"><button className="cp-back" type="button" onClick={()=>setView('Inicio')} aria-label="Volver"><ChevronLeft/></button><div><h1>Crear plan <span aria-hidden="true">✨</span></h1><p>Haz realidad buenos planes</p></div><span className="cp-draft">▣ <b>Borrador</b></span></header>
    <div className="cp-categories cp-photo-categories">{wanted.map(name=><button type="button" key={name} className={category===name?'active':''} onClick={()=>setCategory(name)}><span className="cp-category-photo"><img src={imageFor(name)} alt="" loading="lazy" decoding="async"/></span><span className="cp-category-label"><span>{categoryEmoji[name]}</span><b>{name}</b></span></button>)}</div>
    <form className="cp-form cp-simple-form" onSubmit={e=>void submit(e)}>
      <label className="cp-card cp-wide"><span className="cp-icon"><Sparkles/></span><span className="cp-body"><b>Nombre del plan</b><input value={title} maxLength={60} onChange={e=>setTitle(e.target.value)} placeholder="Ej. Pádel al atardecer" required/><small className="cp-counter">{title.length}/60</small></span></label>
      <label className="cp-card cp-wide"><span className="cp-icon cp-icon-pink"><MapPin/></span><span className="cp-body"><b>Lugar</b><span className="cp-inline cp-location"><input value={place} onChange={e=>setPlace(e.target.value)} placeholder="Tarragona, Salou..." required/><button type="button" aria-label="Usar mi ubicación" onClick={()=>setPlace('Mi ubicación')}>◎</button></span>{place==='Mi ubicación'&&<span className="cp-location-ok">● Ubicación seleccionada</span>}</span></label>
      ...
      <div className="cp-grid"><label className="cp-card"><span className="cp-icon cp-icon-yellow">▰</span><span className="cp-body"><b>Coste</b><input inputMode="decimal" value={cost} onChange={e=>setCost(e.target.value)} placeholder="Opcional €"/></span></label><label className="cp-card"><span className="cp-icon">🔒</span><span className="cp-body"><b>Privacidad</b><select defaultValue="Público"><option>Público</option><option>Solo conexiones</option></select></span></label></div>
      <label className="cp-description cp-simple-description"><span className="cp-icon cp-icon-yellow">▤</span><span className="cp-body"><b>Descripción <small>(opcional)</small></b><textarea maxLength={300} value={description} onChange={e=>setDescription(e.target.value)} placeholder="Cuenta más detalles sobre el plan..."/><small className="cp-counter">{description.length}/300</small></span></label>
      <div className="cp-extras"><button type="button"><Camera/><span><b>Añadir fotos</b><small>0/10</small></span></button><button type="button"><Package/><span><b>Material necesario</b><small>Opcional</small></span></button><button type="button"><span className="cp-car">▱</span><span><b>Transporte</b><small>Opcional</small></span></button></div>
      <button className="cp-submit cp-simple-submit" type="submit" disabled={creating}><Sparkles/><b>{creating?'Publicando…':'Crear plan'}</b><span className="cp-submit-arrow">→</span></button><p className="cp-motto">Un buen plan puede cambiar tu día 💜</p>
    </form>
  </div>;
}
