...
const tests=[
  ...
  ['Filtro de personas usa la vista canónica con filtros',()=>{assert.match(explore,/PeopleGridView/);assert.match(explore,/onFilter=\{peopleState\.setFilter\}/)}],
  ['Like de estados tiene acción',()=>assert.match(explore,/storyState\.toggleLike/)],
  ['Respuesta de estado enlaza con chat',()=>assert.match(explore,/onChat\(/)],
