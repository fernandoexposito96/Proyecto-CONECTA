...
const scanRoots=['src/lib','src/hooks','scripts'];
const failures=[];
const warnings=[];
const sourceExt=/\.(?:ts|tsx|js|mjs)$/;
const patchPattern=/\b(?:TEMP(?:ORARY)?|WORKAROUND|QUICK[ -]?FIX|MONKEY[ -]?PATCH|HOTFIX)\b/i;
const obsoletePattern=/\b(?:DEPRECATED|OBSOLETE)\b/i;
...
for(const file of files){
  ...
  if(patchPattern.test(text))failures.push(`${file}: marcador de parche temporal detectado`);
  if(obsoletePattern.test(text))warnings.push(`${file}: contiene marcador deprecated/obsolete; revisar retirada`);
}
...
console.log(`CONECTA maintainability guard: OK · ${files.length} módulos revisados.`);
