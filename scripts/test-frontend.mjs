...
const checks=[
  ...
  ['Backend conserva COUNT exacto de no leídas',()=>assert.match(notificationsBackend,/count:'exact',head:true/)],
  ['Pantalla recalcula no leídas respetando preferencias',()=>{assert.match(notifications,/fetchBackendNotifications\(200\)/);assert.match(notifications,/rows\.filter\(row=>!row\.read&&toggles\[toggleForType\(row\.type\)\]\)\.length/)}],
  ...
  ['Preferencias de notificación se refrescan con cambios de almacenamiento',()=>{assert.match(notifications,/setToggles\(/);assert.match(notifications,/storageChangeEvent/)}],
  ...
  ['Auth no sobrescribe la nube tras una lectura fallida',()=>{assert.doesNotMatch(authGate,/if\(nextSession\)setCloudStorageWriter\(queueCloudStateSave\)/);assert.match(authGate,/if\(!nextSession\)clearLocalUserState\(\)/)}],
