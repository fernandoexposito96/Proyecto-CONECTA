...
import { CalendarDays, MapPin, Plus, SlidersHorizontal, Sparkles, Star, Users, Zap } from 'lucide-react';
...
export function ExploreView({onPlan,extraPlans=[],initialFilter='all',initialCategory=null,onChat}:{onPlan:(plan:Plan)=>void;extraPlans?:Plan[];initialFilter?:ExploreFilter;initialCategory?:string|null;onChat:(contact:ChatContact)=>void;}){
 const [realPeople,setRealPeople]=useState<PersonSearchResult[]>([]);
 ...
 return <main className="page explore-page explore-live" aria-label="Explora">
  <header className="explore-heading"><div><h1>Explora</h1><p>Conoce gente, descubre planes y vive nuevas experiencias</p></div><div className="explore-tools"><button><MapPin/> Tarragona</button><button aria-label="Filtros"><SlidersHorizontal/></button></div></header>
  <nav className="explore-tabs" aria-label="Filtros de Explora"><button className="active"><Sparkles/>Todo</button><button><Users/>Personas</button><button><CalendarDays/>Planes</button><button><Star/>Eventos</button><button><Users/>Grupos</button></nav>
  ...
  <section className="explore-live-section"><div className="explore-live-title"><h2>Historias de la comunidad</h2><span>Ver todas →</span></div><div className="live-stories">{stories.map((s,i)=><article className="live-story" key={s.name}><img src={s.photo} alt=""/><div className="story-person"><img src={demoPeople[i].photo} alt=""/><span><b>{s.name}</b><small>{s.time}</small></span></div><div className="story-copy"><strong>{s.title}</strong><small><MapPin/> {s.place}</small></div><button>{s.cta}</button></article>)}</div></section>
  ...
  <section className="explore-live-section"><div className="explore-live-title"><h2>Explora por categorías</h2><span>Ver todas →</span></div><div className="explore-categories">{categories.map(([name,photo])=><button key={name}><img src={img(photo)} alt=""/><b>{name}</b></button>)}</div></section>
