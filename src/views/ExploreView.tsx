...
export function ExploreView({onPlan,extraPlans=[],initialFilter='all',initialCategory=null,onChat}:{onPlan:(plan:Plan)=>void;extraPlans?:Plan[];initialFilter?:ExploreFilter;initialCategory?:string|null;onChat:(contact:ChatContact)=>void;}){
 ...
 const plans=useMemo(()=>extraPlans.filter(plan=>!category||normalize(plan.category)===normalize(category)).filter(plan=>planMatchesFilter(plan,initialFilter)).filter(plan=>!query.trim()||normalize(`${plan.title} ${plan.place} ${plan.category}`).includes(normalize(query))).slice(0,20),[extraPlans,category,initialFilter,query]);
