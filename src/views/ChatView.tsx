...
export function ChatView({initialContact=null}:{initialContact?:string|null}){
  ...
  const items=useMemo<ChatItem[]>(()=>{
    const base:ChatItem[]=chats.map(([name,msg,count],i)=>({name,msg,count,isGroup:groupNames.has(name),avatar:avatarFor(name,i)}));
  ...
  if(activeChat){
    const item=items.find(candidate=>candidate.name===activeChat)||{name:activeChat,msg:'Nueva conversación',count:'',isGroup:false,avatar:avatarFor(activeChat)};
