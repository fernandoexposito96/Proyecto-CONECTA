...
export function ProfileView({setView}:{setView:(v:View)=>void}){
  ...
  useEffect(()=>{
    ...
    void supabase.auth.getUser()
      .then(async({data,error})=>{
        ...
        if(!data.user)return;
        const {data:profile,error:profileLoadError}=await supabase.from('profiles').select('bio,identity_status,city,interests,avatar_url').eq('id',data.user.id).maybeSingle();
        ...
        setVerified(String(profile.identity_status||'')==='verified');
        if(typeof profile.city==='string'&&profile.city.trim())setCity(profile.city.trim());
        if(Array.isArray(profile.interests))setInterests(profile.interests.map(value=>String(value)).filter(Boolean));
        if(typeof profile.avatar_url==='string'&&profile.avatar_url)setAvatar(profile.avatar_url);
      })
      .catch(error=>{
  ...
  return <div className="page profile-page">
    <div className="profile-shell">
      <div className="profile-cover"><img loading="lazy" decoding="async" src={`./assets/images/${coverPhotos[coverIndex]}.jpg`} alt="Portada del perfil"/><div className="profile-cover-glow" aria-hidden="true"/><span className="profile-cover-copy" aria-hidden="true">Buenas experiencias<br/>mejores personas ♡</span><button type="button" className="profile-cover-action" onClick={()=>setCoverIndex(i=>(i+1)%coverPhotos.length)}><Camera/> Cambiar portada</button></div>
      <div className="profile-main">
        <div className="profile-avatar-wrap"><img className="profile-avatar" loading="lazy" decoding="async" src={avatar} alt={`Foto de perfil de ${account.name}`}/><span className="profile-online-dot" aria-label="Perfil activo"/></div>
        <div className="profile-heading"><div className="profile-identity"><h1>{account.name}</h1><p className="profile-location"><MapPin/> {locationAllowed?city:'Ubicación oculta a otros usuarios'}</p>{verified&&<span className="profile-verification"><ShieldCheck/> Perfil verificado</span>}</div><div className="profile-actions"><button type="button" className="profile-edit-btn" onClick={()=>{setDraftBio(bio);setProfileError('');setEditing(v=>!v)}}><Pencil/>{editing?'Cancelar':'Editar perfil'}</button><button type="button" className="profile-more-btn" aria-label="Más opciones" onClick={()=>setView('Ajustes')}><MoreHorizontal/></button></div></div>
        {editing?<div className="profile-editor"><label>Biografía<textarea value={draftBio} onChange={e=>setDraftBio(e.target.value)} maxLength={180}/></label><button type="button" disabled={profileSaving} onClick={()=>{void saveProfile()}}>{profileSaving?'Guardando…':'Guardar cambios'}</button></div>:<><p className="profile-bio">{displayBio}</p><div className="profile-interests" aria-label="Intereses">{visibleInterests.map(value=><span key={value}>{value}</span>)}{interests.length>3&&<span>+{interests.length-3}</span>}</div></>}
